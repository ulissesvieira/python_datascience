...
from chap04 import functions as fnt
...
def variance(x):
    ...
    return fnt.sum_of_squares(deviations) / (n - 1)
...
def covariance(x, y):
    n = len(x)
    return fnt.dot(de_mean(x), de_mean(y)) / (n - 1)
...
def correlation(x, y):
    ...
    if stdev_x > 0 and stdev_y > 0:
        ...
        return 0 # if no vatiation, correlation is zero